...
let rec reverseString (i: int) (str: string): char = 
  String.get(str)(i)    (*return the character at the corresponding index value of the string*)
...
let stringrev(cs: string): string = 
  ...
  String.init (length) (fun (i) -> reverseString(length-i-1)(cs)) (*length-i-1 will give the corresponding index of the correct char needed to be inserted into the string such that the string reverses*)